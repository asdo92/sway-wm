...
install_neovim_gen(){
  ...
  neovim_urlfile="https://github.com/neovim/neovim/releases/latest/download/nvim-linux-x86_64.tar.gz"
  ...
  if [[ -f /tmp/neovim.tar.gz ]] ; then
    ...
    rm -rf /usr/bin/nvim
    echo '#!/usr/bin/env zsh' > /usr/bin/nvim
    echo '' >> /usr/bin/nvim
...
if [[ -f /etc/neovim_version.conf ]] ; then
  touch /etc/neovim_version.conf
else
  echo "none" > /etc/neovim_version.conf
fi
